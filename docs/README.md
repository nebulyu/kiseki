...
| [长期目标与路线 MVP](long-term-routes.md) | 目标/路线集中规格：配置加载与查看已实现；证据分析、独立存储、确认与时间线待实现 |
| [开发环境](development/setup.md) | 记录 uv 安装、项目初始化和本地运行约定 |

## 模板原型

模板是可演化的格式样例，不会自动作为个人配置进入运行流程。当前每日分析结构来自 `schema.py`，提示词来自 `model.py`；评分规则 YAML 尚无运行时加载功能。目标/路线配置已支持通过 `route list/show` 加载与查看，默认读取 `private/goals.yaml` 和 `private/routes/*.yaml`，公开模板不自动复制或加载。
