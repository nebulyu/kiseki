...
Phase 2 的日常使用持续进行，下一步并行开发 Phase 3 路线 MVP。目标/路线规格与配置样例已整理；路线加载、路线分析、独立路线 Review 和时间线均尚未实现，本轮没有修改代码架构、数据库或运行配置。
...
- 本次恢复性功能仅使用临时 SQLite、合成 `AnalysisResult` 和本地异常进行自检，没有调用真实模型，也没有触碰真实日记。
...
- 路线开发与数据积累并行，近期范围是配置加载、单篇路线分析、独立 Review 和证据时间线。
- 现有评分 YAML 和目标/路线 YAML 都仍是设计模板，运行中的 `model.py` 不会自动读取它们。
...
下一开发里程碑：按路线规格先实现目标/路线配置加载与列表详情，再接入单篇分析、独立确认和时间线。校准汇总与 UI 保留为后续工作，不阻挡路线 MVP。
