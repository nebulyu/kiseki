...
- 目标与路线先使用 `private/goals.yaml`、`private/routes/*.yaml` 定义；公开样例保存在 `docs/templates/`，当前没有配置加载代码。
- 路线调用读取已有日记原文和所选目标/路线配置，不依赖每日 AI 分析或每日 Review。
...
- 拟议的 `route_cli.py`、`routes.py`、`route_schema.py` 与既有模型/数据库函数协作，交互和计算分开，以便后续 UI 复用。

当前仍只有原来的 Python 模块和数据库结构。本轮只更新文档与模板，不创建新模块、迁移数据库、安装依赖或接入 UI。
